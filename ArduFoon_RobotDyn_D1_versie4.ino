...
//hardware setup
pinMode(DIALTONEOUT   , OUTPUT);        //not uses yet
...
Serial.println  ("");
Serial.println  ("\n\nArdufoon Wonderfoon v 1.4 using RobotDyn D1-r2 WIFI");
...
Serial.println  ("Compiled on December 11th, 2019");
...
if (data.value1>30||data.value2>10) { //failsave for firsttime use of uC
...
//this often doesnt work... to do : read busy pin op MP3 player!
if (playfinished) {
  state = S_IDLE;
}
...
switch (state) {
    ...
    if (digitalRead(FOLDERSELECT)==BUTTONPUSHED) {
        ...
        savesettings();
        //myDFPlayer.volume(dialtonevolume);
        //myDFPlayer.playFolder(99, 98); //beep beep disconnected
